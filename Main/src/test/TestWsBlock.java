...
class TestWsBlock 
{
	...
	void testCalculateLengthSpacesAndTabs() 
	{
		...
		assertEquals(5*4, block7.visual_length);
		assertEquals(4+3+3*4, block8.visual_length);
	}
	...
	void testCalculateLengthAll()
	{
		...
		assertEquals(1, block1.visual_length);
		assertEquals(7, block2.visual_length);
		...
		assertEquals(4, block3.visual_length);
		assertEquals(3, block4.visual_length);
		assertEquals(2, block5.visual_length);
		assertEquals(1, block6.visual_length);
		
		var block7 = new WsBlock("\f\t\t\t\f\t", 5, 5, 1, 1);
		var block8 = new WsBlock("\f\t\t\f\t\f\t\t\f", 8, 9, 8, 2);
		
		// See if subsequent tabs are all of 4 lengths
		assertEquals(3 + 3*4, block7.visual_length);
		assertEquals(5*4, block8.visual_length);
		
		// All characters
		var block9 = new WsBlock(" \t \f \t   \t   \f \t\f", 4, 7, 1, 4);
		var block10 = new WsBlock("\f   \f \t  \f \t  \f\t \t", 5, 6, 8, 5);

		assertEquals(5*4, block9.visual_length);
		assertEquals(4+3+3*4, block10.visual_length);
	}
